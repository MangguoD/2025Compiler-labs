...
/*
1 变量未定义 Exp() 中调用 search(name, 0) 返回 NULL
2 变量重复定义 insert() 前先 ifexist(name, scope_id)
3 函数未定义 check_declar() 最后统一查找 state == DECLA
4 函数重复定义 在 FunDec() 中 ifexist(name, scope_id) 检查
5 函数声明与定义参数不一致 TypeEqual(f1->type, f2->type) 在 insert() 冲突判断中
6 左值错误（不是变量不能赋值） isLeftValue(node) 在 Exp() 分析赋值时调用
7 操作数类型不匹配（如 int + struct） Exp() 中不同操作判断时用 TypeEqual()
8 数组访问非数组变量 Exp() 对 [] 语义节点进行判断 type->kind == ARRAY
9 非整型数组下标 Exp() 判断索引表达式返回类型不是 int
10 对非结构体使用 . 操作 Exp() 判断 Exp -> Exp DOT ID 左侧类型是否 STRUCTURE
11 访问结构体中未定义的字段 search_field(struct_type, fieldname) 是否为 NULL
12 函数调用时实参与形参类型不匹配 TypeEqual(param1->type, param2->type)
13 函数调用时实参与形参数量不匹配 参数链表长度不一致
14 非函数变量使用函数调用 Exp(ID) 返回值不是 FUNCTION 但被调用
15 对非数组变量使用数组赋值 判断 a[1] = ... 中 a 类型是否 ARRAY
16 结构体变量赋值类型不一致 TypeEqual() 返回 0 且类型为 STRUCTURE
17 数组初始化类型不一致 类型为 ARRAY 但 = {} 内元素不匹配（部分简化实验可能略过）
 */
